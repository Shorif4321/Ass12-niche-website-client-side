...
const ManageService = () => {
    ...
    useEffect(() => {
        fetch('https://nameless-cove-93085.herokuapp.com/manage')
            .then(res => res.json())
            .then(data => {
                setManageOrder(data)
                setLoading(false)
    ...
    const handleUpdate = id => {
        const url = `https://nameless-cove-93085.herokuapp.com/status/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
    ...
    const handleManageDelete = id => {
        ...
        if (process) {
            const url = `https://nameless-cove-93085.herokuapp.com/manage/${id}`
            fetch(url, {
                method: 'DELETE',
            })
                .then(res => res.json())
